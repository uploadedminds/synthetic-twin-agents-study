...
all_responses = q.by(sl).by(agents[:150]).by(m).run(disable_remote_inference=True, progress_bar=True, verbose=True)

all_responses.to_pandas().to_csv("../../synthetics_survey_results/gemini_2.5_flash/results.csv", index=False)