...
m = Model("gpt5", service_name = "openai", temperature = 1)

results = q.by(sl).by(agents[:5]).by(m).run(disable_remote_cache=True ,disable_remote_inference=True, progress_bar=True, verbose=True)

output_path = "../../synthetics_survey_results/gpt_5/results_4.csv"
